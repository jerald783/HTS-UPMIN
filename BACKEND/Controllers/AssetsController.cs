...
namespace BACKEND.Controllers
{
    ...
    public class AssetsController : ControllerBase
    {
        ...
        public IActionResult Get()
        {
            ...
            string query = @"SELECT Id,PPE, Fund_Cluster,CollCode, Name, Desc1, 
                            DATE_FORMAT(AnDate, '%Y-%m-%d') AS AnDate, 
                            MrNum, PropNo, Qty, UM, UCost, TCost, UserName, Email, CurrentUser, EqStatus ,SerialNumber,Location,UserId
                            FROM tbl_assets";
        ...
        {
            string query = @"INSERT INTO tbl_assets(PPE,Fund_Cluster,CollCode, Name, Desc1, AnDate, MrNum, PropNo, Qty, UM, UCost, TCost, UserName, Email, CurrentUser, EqStatus,SerialNumber,Location,UserId) 
                             VALUES (@PPE,@Fund_Cluster,@CollCode, @Name, @Desc1, @AnDate, @MrNum, @PropNo, @Qty, @UM, @UCost, @TCost, @UserName, @Email, @CurrentUser, @EqStatus,@SerialNumber,@Location,@UserId)";
            ...
            try
            {
                using (var con = GetConnection())
                {
                    using (var cmd = new MySqlCommand(query, con))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@MrNum", Ass.MrNum);
                        cmd.Parameters.AddWithValue("@PropNo", Ass.PropNo);
        ...
        public IActionResult BulkTransfer([FromBody] BulkTransferRequest request)
        {
            ...
            WHERE Id IN ({string.Join(",", idParams)})";
                ...
                auditCmd.Parameters.AddWithValue("@Details",
                    $"Transferred {rowsAffected} assets (PropNo: {string.Join(", ", propertyNumbers)}) to {request.NewEmail}");
        ...
        {
            string query = @"SELECT Id, Fund_Cluster,CollCode, Name, Desc1, 
                            DATE_FORMAT(AnDate, '%Y-%m-%d') AS AnDate, 
                            MrNum, PropNo, Qty, UM, UCost, TCost, UserName, Email, CurrentUser, EqStatus, SerialNumber,Location,UserId
                     FROM tbl_assets 
