...
export class ItoTicketsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // 🔔 Target the confirmation and diagnostic templates
  @ViewChild('deleteConfirmModal') deleteConfirmModal!: TemplateRef<any>;
  @ViewChild('reopenConfirmModal') reopenConfirmModal!: TemplateRef<any>;
  @ViewChild('dsrResultModal') dsrResultModal!: TemplateRef<any>;
  ...
  // 🩺 Open DSR Diagnostic Result Modal
  openDsrModal(ticket: any): void {
    this.Diagnostic_result = ticket.Diagnostic_result || '';

    const dialogRef = this.dialog.open(this.dsrResultModal, {
      width: '440px',
      data: ticket,
      autoFocus: false,
      backdropClass: ['backdrop-blur-sm', 'bg-slate-900/45'],
...
dialogRef.afterClosed().subscribe((result: any) => {
  if (result) {
    // If result is an object, extract the text field; otherwise use result directly
    const diagnosticText = typeof result === 'object' ? (result.diagnosticResult || result.Diagnostic_Result) : result;

    console.log('Sending ID:', ticket.TicketId, 'Text:', diagnosticText);

    this.tICKET.updateDiagnosticResult(ticket.TicketId, diagnosticText).subscribe({
      next: (res) => {
        ticket.Diagnostic_Result = diagnosticText;
        this.toastr.success('Diagnostic result saved successfully.');
      },
      error: (err) => this.toastr.error('Failed to update database.')
    });
  }
});
  }
