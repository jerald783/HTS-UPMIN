...
namespace BACKEND.Controllers
{
        ...
        public IActionResult GetTicketByUser(string fromemail)
        {
            string query = @"
                SELECT TicketId, TicketNumber, PropNo, FullName, Email, RequestDate,
                       HelpTopic, IssueDesc, Location, PriorityLevel, CurrentStatus,
                       LastUpdated, DueDate, Overdue, AgentAssigned, ReopenCount,
                       FileName, FileContentType, Diagnostic_Result
                FROM tbl_tickets
                WHERE Email = @FromEmail";
    ...
    ";
        ...
        public async Task<IActionResult> AddTicket(TicketModel ticket)
        {
            ...
            string query = @"INSERT INTO tbl_tickets 
                (TicketNumber, PropNo, FullName, Email, RequestDate,
                 HelpTopic, IssueDesc, Location, PriorityLevel,
                 CurrentStatus, LastUpdated, DueDate, Overdue,
                 AgentAssigned, ReopenCount, ExtraFields, Diagnostic_Result)
                VALUES 
                (@TicketNumber, @PropNo, @FullName, @Email, @RequestDate,
                 @HelpTopic, @IssueDesc, @Location, @PriorityLevel,
                 @CurrentStatus, @LastUpdated, @DueDate,
                 @Overdue, @AgentAssigned, 0, @ExtraFields, @Diagnostic_Result)";
            ...
            try
            {
                ...
                using (var cmd = new MySqlCommand(query, con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@AgentAssigned", ticket.AgentAssigned ?? "");
                    cmd.Parameters.AddWithValue("@ExtraFields", ticket.ExtraFields ?? "");
                    cmd.Parameters.AddWithValue("@Diagnostic_Result", ticket.Diagnostic_Result ?? "");
            ...
            try
            {
                ...
                string query = @"INSERT INTO tbl_tickets 
                    (TicketNumber, PropNo, FullName, Email, RequestDate,
                     HelpTopic, IssueDesc, Location, PriorityLevel,
                     CurrentStatus, LastUpdated, DueDate, Overdue,
                     AgentAssigned, ReopenCount, FileName, FileContentType, ExtraFields, Diagnostic_Result)
                    VALUES 
                    (@TicketNumber, @PropNo, @FullName, @Email, @RequestDate,
                     @HelpTopic, @IssueDesc, @Location, @PriorityLevel,
                     @CurrentStatus, @LastUpdated, @DueDate,
                     @Overdue, @AgentAssigned, 0,
                     @FileName, @FileContentType, @ExtraFields, @Diagnostic_Result)";
        ...
        // ============================================================
        [HttpPut("UpdateDiagnosticResult/{id}")]
        public IActionResult UpdateDiagnosticResult(int id, [FromBody] DiagnosticResultDto dto)
        {
            string query = @"
                UPDATE tbl_tickets 
            ...
            try
            {
                ...
                using (var cmd = new MySqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@Diagnostic_Result", dto.Diagnostic_Result ?? string.Empty);
                    cmd.Parameters.AddWithValue("@TicketId", id);
                ...
                return Ok(new { message = "Diagnostic result updated successfully" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, $"Internal Server Error: {ex.Message}");
